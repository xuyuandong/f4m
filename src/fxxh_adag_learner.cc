...
void FxxhAdagLearner::Init(const Parameter& params) {
  Learner::Init(params);
  ...
  std::cout << "Use ad hierarchy fields = ";
...
float FxxhAdagLearner::EvaluateRaw(Example* ex) {
  float raw_y = PredictRaw(ex);
  return (raw_y - w_[0]) * (1.f - drop_rate_) + w_[0];
}

float FxxhAdagLearner::PredictRaw(Example* ex) {
  float r = 1.f;
  ...
  for (size_t x = 0; x < ex->nodes.size(); ++x) {
    ...
    if (ix >= N_ || fx >= M_ || 0 != ads_[fx]) continue;  // no ad field-inter cross
    //std::cout << "x" << fx << ":" <<ix << std::endl;
    ...
    for (size_t y = x+1; y < ex->nodes.size(); ++y) {
      ...
      if (iy >= N_ || fy >= M_ || 0 == ads_[fy]) continue; // only cross with ad 
      //std::cout << "y" << fy << ":" <<iy << std::endl;
  ...
  // bias
  //std::cout << "raw_y " << raw_y << " 0 " << w_[0] << std::endl;
  return raw_y * r + w_[0];
}

void FxxhAdagLearner::Update(Example* ex, float p) {
  float r = 1.f;
  if (norm_) {  // Note: since all feature values are 0 or 1:
  ...
  std::map<int, int> field2index;  // hierarchy mapping
  std::map<int, int> parents;
  for (size_t x = 0; x < ex->nodes.size(); ++x) {
    int ix = ex->nodes[x].index;
    int fx = ex->nodes[x].field;
    field2index[fx] = ix;
    if (ads_[fx] > 0 && field2index.find(ads_[fx]) != field2index.end()) {
      parents[ix] = field2index[ads_[fx]];
    }
  }
  ...
  for (size_t x = 0; x < ex->nodes.size(); ++x) {
    ...
    for (size_t y = x+1; y < ex->nodes.size(); ++y) {
      ...
      float* wp = NULL;  // ad hierarachy
      if (parents.find(iy) != parents.end()) {
        int py = parents[iy];
        int offset = py * Maligned + fx * K_;
        wp = w_.data() + offset; 
      }
      ...
      for (int d = 0; d < K_; d += 4) {
        ...
        __m128 XMMwp = _mm_setzero_ps();
        if (wp != NULL) {  // ad hierarchy sse
          XMMwp = _mm_load_ps(wp+d);
        }
        ...
        __m128 XMMg2 = _mm_add_ps(
            _mm_mul_ps(XMMlambda, _mm_sub_ps(XMMw2, XMMwp)),
...
void FxxhAdagLearner::SaveModel(const Parameter& params) {
...
void FxxhAdagLearner::LoadModel(const Parameter& params) {
  if (params.model_path.empty()) {
    return;
  }
  ...
  if (getline(&line, &length, fp) != (ssize_t)(-1)) {
    ...
    fprintf(stderr, "failed to read YYH model\n");
    exit(1);
  }
