...
#include "third_party/thrift/include/thrift/protocol/TBinaryProtocol.h"
#include "third_party/thrift/include/thrift/protocol/TDebugProtocol.h"
#include "third_party/thrift/include/thrift/protocol/TJSONProtocol.h"
#include "third_party/thrift/include/thrift/transport/TBufferTransports.h"
#include "third_party/thrift/include/thrift/transport/TSocket.h"
#include "third_party/thrift/include/thrift/server/TNonblockingServer.h"
#include "third_party/thrift/include/thrift/concurrency/PosixThreadFactory.h"
#include "third_party/thrift/include/thrift/concurrency/ThreadManager.h"
...
class ThriftNonBlockingServerMutiThread : public Thread {
 ...
 private:
  virtual void Run() {
    ...
    processor_.reset(new ProcessorT(handler_));
    transp_factory_.reset(new TBufferedTransportFactory());
  ...
  boost::shared_ptr<HandlerT> handler_;
  boost::shared_ptr<ProcessorT> processor_;
  boost::shared_ptr<TTransportFactory> transp_factory_;
...
class ThriftClient {
 public:
  ...
  bool GetSocketFd(THRIFT_SOCKET* fd) {
    if (socket_.get() && socket_->isOpen()) {
 ...
 private:
  ...
  void ConnectIfNecessary() {
    if (transport_.get() && transport_->isOpen()) return;
