...
#ifndef _BASE_PRODUCER_CONSUMER_QUEUE_H_ 
#define _BASE_PRODUCER_CONSUMER_QUEUE_H_ 
#include <deque>
  ...
  bool TryGetInternal(Item* item) {
    ...
    while (!que_.empty()) {
      ...
      if (itm.timeout_ == TIMEOUT_NONE ||
        ...
        *item = itm.item_;
        ret = true;
        VLOG(30) << "Normal job" << Timer::CurrentTimeInUs() << ":"
            << itm.timestamp_ << ":" << itm.timeout_;
        break;
      } else {
        VLOG(30) << "Time out job:" << Timer::CurrentTimeInUs() << ":"
            << itm.timestamp_ << ":" << itm.timeout_;
        continue;
      }
    }
...
#endif // _BASE_PRODUCER_CONSUMER_QUEUE_H_ 
