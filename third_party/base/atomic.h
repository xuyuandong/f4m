...
// deprecated for misspelling, pleases use AtomicPointerAssign instead.
template<typename T>
void AtomicPointerAssgin(T* ptr, T new_value) YR_DEPRECATED;
...
// Atomically assign a new value to the pointer.
template<typename T>
void AtomicPointerAssign(T* ptr, T new_value) {
  ignore_result(__sync_lock_test_and_set(ptr, new_value));
}
