...
bool TrimStringTail(const std::string& input,
                    const char trim_chars[],
                    std::string* output);
...
// Same as above FormatBytes() specifing show_units as true,
// But always using the B,KB,MB,GB as the unit suffix.
// The units is calculated by GetByteDisplayUnits().
// Note: the returned string value is an ANSI string.
std::string FormatBytesAndUnitsA(int64 bytes);
std::string FormatBytesAndUnitsA(int64 bytes, DataUnits units);
...
// Same as above FormatSpeed() specifing show_units as true,
// but always using the B/s, KB/s, MB/s, GB/s as the unit suffix.
// The units is calculated by GetByteDisplayUnits().
// Note: the returned string value is an ANSI string.
std::string FormatSpeedAndUnitsA(int64 bytes);
std::string FormatSpeedAndUnitsA(int64 bytes, DataUnits units);
...
bool StringToUint(const string16& input, unsigned int* output);
...
unsigned int StringToUint(const string16& value);
